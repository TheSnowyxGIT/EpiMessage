...
function get_init_data(session, server_data, callback){
    ...
    ddb_epinotes.get_users((err_users, users)=>{
        if (err_users == null){
            obj["users"] = users;
            obj["me"] = {};
            obj["users_connected"] = users_online_methodes.get_online_users(users_socket);
            ...
            ddb_epinotes.get_user(user_id, (err_user, user)=>{
                if (err_user == null){
                    ...
                    obj.me['id'] = user_id;
                    obj.me['user'] = user;
                    ddb_epimessage.get_last_msg_both_per_user(user_id, (err_lastmsgs_private, last_msgs_private)=>{
                        if (err_lastmsgs_private == null){
                            ...
                            ddb_epimessage_groups.get_groups_of_user(user_id ,(err_groups, groups)=>{
                                if (err_groups == null){
                                    obj["groups"] = groups;
                                    ddb_epimessage_groups.get_last_message_per_groups_user(user_id, (err_lastmsgs_groups, last_msgs_groups)=>{
