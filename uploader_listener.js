...
module.exports = function(socket, server_data) {
            ...
            if (err_socket != null){
                ...
                if (server_data.spam.is_allow_to_send(session.Id)){
                    if (event.file.size > maxFileSize){
                        socket.emit("error", {code: error.code.UPLOAD_MAX_SIZE, msg: error.get_error(error.code.UPLOAD_MAX_SIZE, {
    ...
    uploader.on("complete", function(event){
        ...
        sessions.type_socket(socket, (err_socket, session) => {
            if (err_socket == null){
                ...
                magic.detectFile(file_gestion.files_path_tmp+file_name, function(err, file_mime) {
                    ...
                    if (!err){
                        ...
                        if (data.type == "img"){
                            if (types_allowed["img"].includes(file_mime)){
                                if (data.channel_type == "private"){
                                    send_private_img_msg(socket, users_socket, data, session, file_name);
                                }else if (data.channel_type == "group"){
                                    send_group_img_msg(socket, users_socket, data, session, file_name);
                                }else{
                                ...
                                file_gestion.delete_tmp_file(file_name);
                                socket.emit("error", {code: error.code.FILE_TYPE_INCORRECT, msg: error.get_error(error.code.FILE_TYPE_INCORRECT, {
                            ...
                            if (types_allowed["file"].includes(file_mime)){
                                if (data.channel_type == "private"){
                                    send_private_file_msg(socket, users_socket, data, session, file_name);
                                }else if (data.channel_type == "group"){
                                    send_group_file_msg(socket, users_socket, data, session, file_name);
                                }else{
                                    file_gestion.delete_tmp_file(file_name);
                                    socket.emit("error", {code: error.code.UNKNOWN_CHANNEL_TYPE, msg: error.get_error(error.code.UNKNOWN_CHANNEL_TYPE, {
                                        type: data.channel_type
                            ...
                            if (types_allowed["video"].includes(file_mime)){
                                if (data.channel_type == "private"){
                                    send_private_video_msg(socket, users_socket, data, session, file_name);
                                }else if (data.channel_type == "group"){
                                    send_group_video_msg(socket, users_socket, data, session, file_name);
                                }else{
                                    file_gestion.delete_tmp_file(file_name);
                                    socket.emit("error", {code: error.code.UNKNOWN_CHANNEL_TYPE, msg: error.get_error(error.code.UNKNOWN_CHANNEL_TYPE, {
                                        type: data.channel_type
                        ...
                        console.log(err)
                        file_gestion.delete_tmp_file(file_name);
                        socket.emit("error", {code: error.code.GET_MIME_OF_FILE, msg: error.get_error(error.code.GET_MIME_OF_FILE)});
                    }
                });
            }else{
                file_gestion.delete_tmp_file(file_name);
    ...
    uploader.on("error", function(event){
        ...
        if (error_msg == "Error: Max allowed file size exceeded"){
            ...
            socket.emit("error", {code: error.code.UPLOAD_FILE, msg: error.get_error(error.code.UPLOAD_FILE,{
    ...
    file_gestion.get_new_name_file(file_name, (err_hash, new_file_name)=>{
        if (err_hash == null){
            file_gestion.move_tmp_file(file_name, new_file_name, err_move =>{
                if (err_move == null){
                    let img_sql = data_format.image_sql_format(new_file_name, data.mime_type);
                    ddb_epimessage.send_private_img(session.Id, data.id_receiver, img_sql, (err_send)=>{
                      if (err_send == null){
                        ddb_permissions.Give_access_to_file_2users(session.Id, data.id_receiver, new_file_name, (err_perm)=>{
                            if (err_perm == null){
                                const msg = data_format.make_client_img_private_format(session.Id, data.id_receiver, data_format.get_url_from_filename(new_file_name), data.mime_type);
                                ...
                                socket.emit("error", {code: error.code.INSERT_PERMISSIONS, msg: error.get_error(error.code.INSERT_PERMISSIONS)});
                            }
                        });
                      }else{
                        file_gestion.delete_file(new_file_name);
                    ...
                    file_gestion.delete_tmp_file(file_name);
                    socket.emit("error", {code: error.code.MOVING_RENAMING_FILE, msg: error.get_error(error.code.MOVING_RENAMING_FILE)});
                }
            });
        }else{
            file_gestion.delete_tmp_file(file_name);
            socket.emit("error", {code: error.code.HASH_FILE, msg: error.get_error(error.code.HASH_FILE)});
        }
    });
}
            ...
            file_gestion.get_new_name_file(file_name, (err_hash, new_file_name)=>{
                if (err_hash == null){
                    file_gestion.move_tmp_file(file_name, new_file_name, err_move =>{
                        if (err_move == null){
                            let img_sql = data_format.image_sql_format(new_file_name, data.mime_type);
                            ddb_epimessage_groups.send_groups_img(session.Id, data.group_id, img_sql, (err_send)=>{
                              if (err_send == null){
                                ddb_permissions.Give_group_access_to_file(data.group_id, new_file_name, (err_perm)=>{
                                    if (err_perm == null){
                                        const msg = data_format.make_client_img_group_format(session.Id, data.group_id, data_format.get_url_from_filename(new_file_name), data.mime_type);
                                        socket.emit("new_private_msg", msg);
...
function send_private_file_msg(socket, users_socket, data, session, file_name){
    file_gestion.get_new_name_file(file_name, (err_hash, new_file_name)=>{
        if (err_hash == null){
            file_gestion.move_tmp_file(file_name, new_file_name, err_move =>{
                if (err_move == null){
                    let file_sql = data_format.file_sql_format(file_name, new_file_name, data.mime_type);
                    ddb_epimessage.send_private_file(session.Id, data.id_receiver, file_sql, (err_send)=>{
                      if (err_send == null){
                        ddb_permissions.Give_access_to_file_2users(session.Id, data.id_receiver, new_file_name, (err_perm)=>{
                            if (err_perm == null){
                                const msg = data_format.make_client_file_private_format(session.Id, data.id_receiver, data_format.get_url_from_filename(new_file_name), file_name, data.mime_type);
                                socket.emit("new_private_msg", msg);
                                if (users_socket[data.id_receiver] != undefined){
                                    users_socket[data.id_receiver].emit("new_private_msg", msg);
                                }
            ...
            file_gestion.get_new_name_file(file_name, (err_hash, new_file_name)=>{
                if (err_hash == null){
                    file_gestion.move_tmp_file(file_name, new_file_name, err_move =>{
                        if (err_move == null){
                            let file_sql = data_format.file_sql_format(file_name, new_file_name, data.mime_type);
                            ddb_epimessage_groups.send_groups_file(session.Id, data.group_id, file_sql, (err_send)=>{
                              if (err_send == null){
                                ddb_permissions.Give_group_access_to_file(data.group_id, new_file_name, (err_perm)=>{
                                    if (err_perm == null){
                                        const msg = data_format.make_client_file_group_format(session.Id, data.group_id, data_format.get_url_from_filename(new_file_name), file_name, data.mime_type);
                                        socket.emit("new_private_msg", msg);
                                        socket.broadcast.emit("new_private_msg", msg);
                                    }else{
                                        file_gestion.delete_file(new_file_name);
...
function send_private_video_msg(socket, users_socket, data, session, file_name){
    file_gestion.get_new_name_file(file_name, (err_hash, new_file_name)=>{
        if (err_hash == null){
            file_gestion.move_tmp_file(file_name, new_file_name, err_move =>{
                if (err_move == null){
                    let video_sql = data_format.video_sql_format(file_name, new_file_name, data.mime_type);
                    ddb_epimessage.send_private_video(session.Id, data.id_receiver, video_sql, (err_send)=>{
                      if (err_send == null){
                        ddb_permissions.Give_access_to_file_2users(session.Id, data.id_receiver, new_file_name, (err_perm)=>{
                            if (err_perm == null){
                                const msg = data_format.make_client_video_private_format(session.Id, data.id_receiver, data_format.get_url_from_filename(new_file_name), file_name, data.mime_type);
                                socket.emit("new_private_msg", msg);
                                if (users_socket[data.id_receiver] != undefined){
                                    users_socket[data.id_receiver].emit("new_private_msg", msg);
                                }
            ...
            file_gestion.get_new_name_file(file_name, (err_hash, new_file_name)=>{
                if (err_hash == null){
                    file_gestion.move_tmp_file(file_name, new_file_name, err_move =>{
                        if (err_move == null){
                            let video_sql = data_format.video_sql_format(file_name, new_file_name, data.mime_type);
                            ddb_epimessage_groups.send_groups_video(session.Id, data.group_id, video_sql, (err_send)=>{
                              if (err_send == null){
                                ddb_permissions.Give_group_access_to_file(data.group_id, new_file_name, (err_perm)=>{
                                    if (err_perm == null){
                                        const msg = data_format.make_client_video_group_format(session.Id, data.group_id, data_format.get_url_from_filename(new_file_name), file_name, data.mime_type);
                                        socket.emit("new_private_msg", msg);
                                        socket.broadcast.emit("new_private_msg", msg);
                                    }else{
                                        file_gestion.delete_file(new_file_name);
