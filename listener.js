...
module.exports = function (socket, server_data) {
  ...
  socket.on("send_message", (data) => {
    sessions.type_socket(socket, (err_socket, session) => {
      if (err_socket == null) {
        ...
        if (data.type == "text") {
          if (data_received_check.check_textmsg(data)) {
            ...
            socket.emit("error", {
              code: error.code.SENDING_WRONG_DATA,
              msg: error.get_error(error.code.SENDING_WRONG_DATA),
            });
          }
        } else {
          socket.emit("error", {
  ...
  socket.on("ask_private_msgs", (data) => {
    sessions.type_socket(socket, (err_socket, session) => {
      if (err_socket == null) {
        if (!data_received_check.check_askmsg_private(data)) {
          // CHECK DATA STRUCTURE
          socket.emit("error", {
            code: error.code.DATA_INCORRECT,
            msg: error.get_error(error.code.DATA_INCORRECT),
          });
          return;
        }

        ddb_epimessage.get_private_msgs(
          session.Id,
          data.user,
          new Date(),
          (err_msg, msgs) => {
            if (err_msg == null) {
                ...
                if (type == "text") {
                  ...
                  formated_data =
                    data_format.make_client_img_private_format_all(
                      row.id_sender,
                      row.id_receiver,
                      data_format.get_url_from_filename(img_sql_data.file_name),
              ...
              console.log(err_msg);
              socket.emit("error", {
                code: error.code.LOADING_PRIVATE_MSGS,
                msg: error.get_error(error.code.LOADING_PRIVATE_MSGS),
              });
            }
          }
        );
  ...
  socket.on("ask_group_msgs", (data) => {
    sessions.type_socket(socket, (err_socket, session) => {
      if (err_socket == null) {
        if (!data_received_check.check_askmsg_group(data)) {
          // CHECK DATA STRUCTURE
          socket.emit("error", {
            code: error.code.DATA_INCORRECT,
            msg: error.get_error(error.code.DATA_INCORRECT),
        ...
        ddb_groups_permissions.Has_access_to_group(
          ...
          (err_perm, has_access) => {
            if (err_perm == null && has_access) {
              ddb_epimessage_groups.get_group_msgs(
                data.group_id,
                new Date(),
                (err_msg, msgs) => {
                  if (err_msg == null) {
  ...
  socket.on("read_message", (data) => {
    sessions.type_socket(socket, (err_socket, session) => {
      if (err_socket == null) {
        ddb_epimessage.set_private_msgs_readed(
          session.Id,
          data.id_sender,
          (err_read) => {}
        );
...
function send_private_text_msg(socket, users_socket, data, session) {
  ddb_epimessage.send_private_msgs(
    session.Id,
    data.id_receiver,
    data.message_body,
    (err_send) => {
      if (err_send == null) {
        const msg = data_format.make_client_msg_private_format(
          session.Id,
          data.id_receiver,
          data.message_body
        );
...
function send_group_text_msg(socket, users_socket, data, session) {
  ddb_groups_permissions.Has_access_to_group(
    ...
    (err_perm, has_access) => {
      if (err_perm == null && has_access) {
        ddb_epimessage_groups.send_groups_msgs(
          session.Id,
          data.group_id,
          data.message_body,
          (err_send) => {
            if (err_send == null) {
              const msg = data_format.make_client_msg_group_format(
                session.Id,
                data.group_id,
                data.message_body
              );
